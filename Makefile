...
test-unit:
	docker run --rm --volume ${PWD}:/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest sh -c "pytest --cov --cov-report=xml:results/coverage.xml --cov-report=html:results/coverage --junit-xml=results/unit_result.xml -m unit" || true
	docker run --rm --volume ${PWD}:/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest sh -c "junit2html results/unit_result.xml results/unit_result.html"
test-behavior:
...
test-e2e:
	...
	docker run --rm --volume ${PWD}/test/e2e/cypress.json:/cypress.json --volume ${PWD}/test/e2e/cypress:/cypress --volume ${PWD}/results:/results  --network calc-test-e2e cypress/included:4.9.0 --headless || true
...
test-e2e-wiremock:
	...
	docker run --rm --volume ${PWD}/test/e2e/cypress.json:/cypress.json --volume ${PWD}/test/e2e/cypress:/cypress --volume ${PWD}/results:/results --network calc-test-e2e-wiremock cypress/included:4.9.0 --headless || true
